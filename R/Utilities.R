...
funcg = function (x,bet,alph,b,deld,gam){
  ...
  if(x <= 0){
    ...
    xb=deld*sqrt(x)
    if (abs(xb) <= 30){ g=1+(b/sqrt(2*pi))*sqrt(x)*exp(-b*b*x/8)+
...
#'\item{4: \code{eps1} = 20\%}
#'\item{5: \code{eps1} = 25\%}}
#'The critical values are tabulated from @references Perron and Bai, 1998
...
# check supplied formula if it's conform
#' @importFrom stats formula model.response terms
#' @noRd
check_formula <- function(f = f){
  if(!inherits(f,"formula")){
    stop('Please use formula to describe structural break model symbolically')
  }
  if(length(f) != 3){
    stop('Invalid formula. Need an expression of the form y ~ z | x or y ~ z')
  }
  #manipulate RHS based on string representation of the formula
  rhs = f[[3]]
  rhs.tkns = strsplit(as.character(list(rhs)),' ')
  rhs.tkns = unlist(rhs.tkns)
  #check number of separator "|"
  sep_idx = grep(rhs.tkns,pattern = "|", fixed =TRUE)
  if (length(sep_idx) == 0){
    message('No x regressors')
    final.f <- f #simpliest case, no x regressors, all z
    f.terms <- terms(f)
    attr(final.f,"z-intercept") <- attr(f.terms,"intercept")
    attr(final.f,"x-intercept") <- 0
    attr(final.f,".Environment") <- attr(f.terms,".Environment")
    attr(final.f,"z-regs") <- attr(f.terms,'term.labels')
    attr(final.f,"x-regs") <- NA
  } else if(length(sep_idx) > 1) {
    stop('Invalid formula. You can only have at most 1 regressors seperator "|".')
  } else{
    y.str <- as.character(f[[2]])
    #now form 2 formulae based on the separators and conduct checks
    z.tkns <- rhs.tkns[1:sep_idx-1]
    z.str <- paste(z.tkns,collapse = ' ')
    z.formula <- eval(formula(paste(y.str,'~',z.str,collapse = ' ')))
    z.terms <- terms(z.formula)
    z.terms.label <- attr(z.terms,'term.labels')
    x.tkns <- rhs.tkns[(sep_idx+1):length(rhs.tkns)]
    x.str <- paste(x.tkns,collapse = ' ')
    x.formula <- eval(formula(paste(y.str,'~',x.str,collapse = ' ')))
    x.terms <- terms(x.formula)
    x.terms.label <- attr(x.terms,'term.labels')
    common.terms <- intersect(z.terms.label,x.terms.label)
    if (length(common.terms) > 0){
      stop(paste('Duplicate regressor(s):',common.terms))
    }
    
    if (length(z.terms.label) == 0 && attr(z.terms, "intercept")  == 0){
      stop('No z regressors. Use lm() instead')
    } 
    
    final.f <- paste(y.str,'~',paste(z.terms.label,collapse='+'),
                     '+',paste(x.terms.label,collapse='+'),collapse=' ')
    final.f <- formula(final.f)
    attr(final.f, ".Environment") <- attr(f, ".Environment")
    attr(final.f, "z-intercept") <- attr(z.terms, "intercept")
    attr(final.f, "x-intercept") <- attr(x.terms, "intercept")
    attr(final.f, "z-regs") <-  attr(z.terms, "term.labels")
    attr(final.f, "x-regs") <-  attr(x.terms, "term.labels")
  }
  
  #final check if intercepts appear in both z and x regressors
  #intercept over z regressors are going to take priority
  if(attr(final.f,"z-intercept") == 1 && attr(final.f,"x-intercept") == 1){
    message("Formula has both intercept in z and x regressors. Intercept in z regressors will take priority")
    attr(final.f,"x-intercept") <- 0
  }
...
# finding significant level code
#' @noRd
findSignif <- function(v,cv){
  sigcode <- c('','.','*','**','***')
  out <- v
  for (i in (1:length(out))){
  if (v[i] <= cv[1]) out[i] <- sigcode[1]
  else if (v[i] <= cv[2]) out[i] <- sigcode[2]
  else if (v[i] <= cv[3]) out[i] <- sigcode[3]
  else if (v[i] <= cv[4]) out[i] <- sigcode[4]
  else out[i] <- sigcode[5]}
  out
}
