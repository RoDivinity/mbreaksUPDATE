...
#'@param eps1 trimming level. The default value for `eps1` is set at 0.15.
...
doglob = function (y,z,x,m,eps,h,maxi,fixb,betaini,printd,eps1=0.15){
...
#' and critical values. The default value is 0.15. Minimal segment length `h` will be set
...
#' }
#'@param eps convergence criterion for iterative recursive computation (Only required
#'for partial structural change model estimation)
...
doseqtests = function(y_name,z_name=NULL,x_name=NULL,data,
  ...
  rownames(cv_supFl) = paste(siglev,'%',sep='')
  colnames(cv_supFl) = paste('m=',c(1:m))
...
#' @param eps1 value of trimming (in percentage) for the construction
#' and critical values. The default value for `eps1` is set at 0.15. Minimal segment length `h` will be set
#' at \code{default} = int(\code{eps1}*T) (T is total sample size).
#  There are five options:
#' \itemize{
#' \item{`eps1=0.05` Maximal value of `m` = 10}
#' \item{`eps1=0.10` Maximal value of `m` = 8}
#' \item{`eps1=0.15` Maximal value of `m` = 5}
#' \item{`eps1=0.20` Maximal value of `m` = 3}
#' \item{`eps1=0.25` Maximal value of `m` = 2}
...
if(eps1 == 0){
  stop('The estimation procedure is invalid since it requires specific trimming level for testing, but eps1 is set to 0')
}
...
dorepart = function(y_name,z_name = NULL,x_name = NULL,data,
  ...
  #estimate the date at significant level specified by users
  mSEL = nbreak
...
print.sbtests <- function(x,digits = 3,...)
{ if(x$mbreak == 0){
  warning('\nThe test is undefined for no break model\n')
}else{
  cat('\na) SupF tests against a fixed number of breaks\n\n')
  temp_sigcode <- x$ftest
  for (j in 1:length(x$ftest)){
    temp_sigcode[j] <- findSignif(x$ftest[j],x$cv_supF[,j])
  }
  rownames(temp_sigcode) <- colnames(x$supF1[1,])
  temp.tab <- rbind(x$supF1[1,],t(temp_sigcode))
  rownames(temp.tab) <- c('S.SupF','Sig. lev.')
  print(temp.tab,quote=FALSE)
  cat('\nb) UDmax tests against an unknown number of breaks\n\n')
  temp_sigcode <- findSignif(max(x$ftest),x$cv_Dmax)
  temp.tab2 <- cbind(round(max(x$ftest),digits,...),temp_sigcode)
  colnames(temp.tab2) <- c('UDMax','Sig. lev.') 
  print(temp.tab2,quote=FALSE)
  cat('---\n')
  cat('Signif. codes: ', paste(c("0.01","0.025","0.05","0.10",">0.10"),c("\'***\'","\'**\'","\'*\'","\'.\'","\' \'"),collapse= '  '))
 }
  invisible(x)
}

#' @export
summary.sbtests <- function(x,...)
{ if(x$mbreak == 0){
  warning('\nThe test is undefined for no break model\n')
}else{
  cat('\na) SupF tests against a fixed number of breaks\n\n')
...
print.seqtests = function(x,...){
  temp_sigcode <- x$supfl
  for (j in 1:length(x$supfl)){
    temp_sigcode[j] <- findSignif(x$supfl[j],x$cv[,j])
  }
  rownames(temp_sigcode) <- colnames(x$sfl[1,])
  cat('\nsupF(l+1|l) tests using global optimizers under the null\n\n')
  temp.tab <- rbind(x$sfl[1,],t(temp_sigcode))
  rownames(temp.tab) <- c('S.SupF','Sig. lev.')
  print(temp.tab)
  cat('---\n')
  cat('Signif. codes: ', paste(c("0.01","0.025","0.05","0.10",">0.10"),c("\'***\'","\'**\'","\'*\'","\'.\'","\' \'"),collapse= '  '))
